<div>
            ...
            <table class="min-w-full bg-white">
                <thead>
                    <tr class="bg-blue-100 text-left text-gray-700 font-semibold">
                        ...
                        <th class="px-4 py-3 border-b text-center">Nama Almarhum</th>
                        <th class="px-4 py-3 border-b text-center">Jenis Kelamin</th>
                        <th class="px-4 py-3 border-b text-center">Tanggal Meninggal</th>
                        <th class="px-4 py-3 border-b text-center">Jam Kematian</th>
                        <th class="px-4 py-3 border-b text-center">Tempat Pemakaman</th>
                ...
                <tbody>
                    @forelse ($kematianList as $index => $item)
                        <tr class="border-t hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-4 py-3 font-medium text-gray-900 text-center">{{ $item->nama_warga }}</td>
                            ...
                            <td class="px-4 py-3 text-center text-gray-600">
                                {{ \Carbon\Carbon::parse($item->tanggal_meninggal)->format('d M Y') }}
                            </td>
                            <td class="px-4 py-3 text-center text-gray-600">
                                <span class="font-mono">{{ $item->jam_meninggal }}</span>
                            </td>
                            <td class="px-4 py-3 text-gray-700 text-center">
                                {{ $item->tempat_pemakaman ?? '-' }}
                            </td>
                            <td class="px-4 py-3 text-gray-600 text-center">{{ $item->pelapor->rw->no_RW }}</td>
                            <td class="px-4 py-3 text-gray-600 text-center">{{ $item->pelapor->rt->no_RT }}</td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    @if (!$item->file_ttd_admin)
                                        {{-- Tombol Edit --}}
                                        <button wire:click="showEdit({{ $item->id_kematian }})"
                                            ...
                                            <svg wire:loading wire:target="showEdit"
                                                class="w-4 h-4 animate-spin text-white" fill="none"
                                                viewBox="0 0 24 24">
                                                <circle class="opacity-25" cx="12" cy="12" r="10"
                                                    stroke="currentColor" stroke-width="4" />
                                                <path class="opacity-75" fill="currentColor"
                                        ...
                                        {{-- Tombol Hapus --}}
                                        <button wire:click="showDelete({{ $item->id_kematian }})"
                        ...
                        <tr>
                            <td colspan="7" class="text-center text-gray-500 py-8">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <span class="font-medium">Belum ada surat kematian</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <!-- Modal -->
    <div class="space-y-4">
        @livewire('surat.kematian.detail-kematian')
        @livewire('surat.kematian.edit-kematian')
        @livewire('surat.kematian.delet-kematian')
    </div>
